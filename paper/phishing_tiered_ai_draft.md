...
**Classifier.** We use a Random Forest [4] over the 87-feature representation. Hyper-parameters are chosen by grid search *inside* a k=5 cross-validation, with an *outer* locked test split (nested cross-validation) used only for the numbers in Section VI-A, so model selection never sees the evaluation data. Feature reduction to the strongest ~30–42 features uses **permutation** importance measured on held-out folds (impurity importance is biased toward high-cardinality features); a compact model is then retrained, following the importance-driven methodology of [5] and general practice [1], [31]. Three features that are constant in this dataset release (`sfh`, `ratio_intErrors`, `ratio_intRedirection`) are dropped up front. Two further features are kept but flagged non-stationary: `web_traffic` (Alexa-derived; Alexa ranks retired May 2022, so it cannot be reproduced live and is imputed as "unknown" at inference time), and `google_index`, which had the strongest univariate association with the label in our inspection but is exactly the kind of signal that drifts — search engines de-index known phishing quickly, so its training-time and deployment-time distributions differ.
...
1. **Domain age.** Query RDAP first, falling back to WHOIS. RDAP returns structured JSON, and its query format, JSON responses, and bootstrap procedure are standardized [16], [17], [15]; since 28 January 2025 ICANN no longer requires gTLD registries to run port-43 WHOIS, making RDAP the primary path for those TLDs [18]. The IANA RDAP bootstrap registry is consulted to find the authoritative server per TLD [15]. `.vn` is not currently served by RDAP, so Vietnamese domains fall through to WHOIS in practice — an expected outcome, not an error. Each step has a short timeout (target ≈ 500 ms). If both sources return nothing — including deliberate registration-privacy redaction — the rule reports "unknown," which is *not* treated as safe.
2. **TLS certificate.** A missing or invalid certificate, or one issued within the last two days, raises a flag; very fresh certificates correlate with disposable phishing infrastructure.
3. **Typosquatting.** Levenshtein edit distance [21] between the registrable domain and each entry in a curated Vietnamese brand-domain list (banks, e-commerce platforms, e-wallets); measurement studies of typosquatting abuse motivate edit-distance screening [37], [38]. The acceptance threshold scales with brand-name length as `max(1, floor(len(brand)/5))`, so short brand names do not generate false matches. Edit distance alone misses homoglyph substitutions (e.g. Cyrillic look-alikes); a skeleton/confusable normalization before the comparison is a planned addition, and the false-match rate of the rule is reported in Section VI-A.
...
No results for the proposed system are reported here. This plan — metrics, numeric targets, and pass/fail criteria — is fixed in advance so that later results cannot be selectively framed. Table 3 collects the targets; they are design goals, not measurements.
...
- **No empirical results yet.** This paper is a design and a protocol; the numbers that would substantiate a detection claim are not in it.
